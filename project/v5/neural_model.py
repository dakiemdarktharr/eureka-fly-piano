"""Connectome-constrained rate network + explicitly engineered task relay, feedback and decoder."""
...
class MotorNetwork:
    ...
    def step(self,drive,key,feedback=None,cue=True,goal_drive=None):
        target=np.zeros(8)
        ...
        if goal_drive is not None:
            inp[self.e1]+=np.clip(np.asarray(goal_drive,float),0,50) # Engineered local target cue; not a measured pathway
