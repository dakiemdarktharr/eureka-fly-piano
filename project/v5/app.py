...
def readiness():
    required=[ROOT/'assets/manc.json',PROJECT/'connectome/weights.npy',PROJECT/'connectome/neurons.csv',V2/'data/ik_cache.npz',V2/'data/merry_score.json',V2/'data/pool_score.json',V2/'assets/fly_model/piano_fly.xml']
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=='/api/health':return self.json({'ok':True,'version':5,**readiness()})
        if path=='/api/export':
...
def main():
    ...
    try:server=ThreadingHTTPServer((a.host,a.port),Handler)
        ...
        try:
            ...
            if health.get('version')!=5:raise RuntimeError('Port is used by another app')
    ...
    print(f'Fly Piano v5: http://127.0.0.1:{a.port}/',flush=True);server.serve_forever()
