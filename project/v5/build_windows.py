...
args=[sys.executable,'-m','PyInstaller','--noconfirm','--onedir','--windowed','--name','FlyPianoLabV5','--distpath',str(ROOT/'dist'),'--workpath',str(ROOT/'build'),'--specpath',str(ROOT/'build'),'--paths',str(ROOT),'--paths',str(PROJECT/'v2'),'--paths',str(PROJECT),'--collect-all','mujoco','--hidden-import','scipy.optimize','--hidden-import','psutil','--collect-submodules','scipy._external.array_api_compat','--collect-submodules','scipy._external.array_api_extra']
...
for p in ['output','research']:
 if p=='output' and (ROOT/p).exists():add(ROOT/p,'bundle/project/v5/'+p)
if (ROOT/'research/cpu_benchmark.json').exists():add(ROOT/'research/cpu_benchmark.json','bundle/project/v5/research')
...
print(ROOT/'dist/FlyPianoLabV5/FlyPianoLabV5.exe')
