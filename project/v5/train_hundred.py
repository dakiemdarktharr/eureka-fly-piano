...
def clip(notes, offset, width=None):
    # Exactly 100 original events, including duplicate/unassigned targets.
    if len(notes)<100: raise ValueError('At least 100 target events required')
    start=max(0,min(int(offset),len(notes)-100))
    chosen=notes[start:start+100]; origin=chosen[0]['start']
    selected=[dict(n,start=n['start']-origin+.4,end=n['end']-origin+.4) for n in chosen]
    return selected,max(n['end'] for n in selected)+.35
...
def run(minutes=120, workers=4):
    ...
    initial = np.r_[np.asarray(source['parameters'], float),np.zeros(6)]
    warm = read_json(STATE/'continuation.json') if (STATE/'continuation.json').exists() and not (STATE/'latest.json').exists() else {}
    ...
    init(); start = time.monotonic(); budgets={k:max(0,minutes*60-warm.get(k,{}).get('consumed_s',0)) for k in ('merry','pool')}; total=sum(budgets.values())
    config = dict(version=5, kind='song_practice', run_id=rid, minutes_per_song=minutes,
                  budget_s=total, workers=workers, songs=['merry','pool'], warm_start=source,
                  precision_gate=None, optimizer='CEM', population=8, elite=3,
                  parameter_names=NAMES, seed=0, dt=.0002, targets_per_episode=100, continuation=warm, remaining_budget_s=budgets,
                  ...
                  hashes={str(p.relative_to(PROJECT)):sha(p) for p in [ROOT/'engine.py',ROOT/'train_hundred.py',V2/'world.py',ROOT/'neural_model.py',PROJECT/'connectome/weights.npy',PROJECT/'connectome/neurons.csv',V2/'data/ik_cache.npz',V2/'assets/fly_model/piano_fly.xml',V2/'data/merry_score.json',V2/'data/pool_score.json']},
    ...
    if not (old/'skill.json').exists(): old=STATE/'preview'
    ...
    try:
        with concurrent.futures.ProcessPoolExecutor(workers,mp_context=multiprocessing.get_context('spawn'),initializer=init) as pool:
            for index,c in enumerate(cells):
                if stopped(): break
                key=c['song']; song_start=time.monotonic(); deadline=song_start+budgets[key]
                train_end=song_start+budgets[key]*.90
                initial=np.asarray(warm.get(key,{}).get('parameters',config['warm_start']['parameters']+[0.]*6),float)
                if len(initial)==49: initial=np.r_[initial,np.zeros(6)]
                c['parameters']=initial.tolist();c['warm_start_parameters']=initial.tolist();c['prior_consumed_s']=warm.get(key,{}).get('consumed_s',0)
                notes,duration=SONGS[key]; bins=sorted(set(list(range(0,len(notes)-99,100))+[len(notes)-100]))
                monitor=[bins[len(bins)//2]]
                c['monitor_start_indices']=monitor; c['state']='training'
                ...
                while time.monotonic()<train_end and not stopped():
                    if not pending: pending=[int(x) for x in rng.permutation(bins)]
                    ...
                    c['last_episode_targets']=[r.get('metric',{}).get('target_notes') for r in rr]
                    ...
                    if len(good)<3: break
                    covered.update(n['id'] for n in clip(notes,offset)[0])
                    ...
                    c['generation']+=1
                    atomic_json(folder/f'{key}_optimizer.json',dict(mean=mean.tolist(),sigma=sigma.tolist(),rng=rng.bit_generator.state,pending=pending,best=best.tolist(),generation=c['generation'],protocol='100-target CEM'))
                    if c['generation']%5==0:
                        vm=measure(mean)
                        if vm:
                            c['validation']=vm
                    ...
                    save('training',f"{key}: 100 nốt/lượt; thế hệ {c['generation']}; đã luyện {len(covered)}/{len(notes)} nốt; không dừng theo precision.")
                ...
                for label,theta,record in [('baseline',initial,False),('trained',best,True)]:
                    ...
                    if record:
                        ...
                        atomic_json(folder/'replay_manifest.json',manifest)
                # One fixed 100-target skill diagnostic, never used to select weights.
                skill_notes,skill_duration=E.synthetic(91002,count=100)
                try:
                    diagnostic,trace=E.rollout(best,skill_notes,skill_duration,record=True,cancel=lambda:time.monotonic()>=deadline)
                    c['evaluation_steps']+=diagnostic['physics_steps']
                    diagnostic.update(piece='skill',diagnostic=True,checkpoint_label=f'{key} / {rid}',evaluation_scope='100 synthetic targets; fixed seed 91002; never selected on this result')
                    trace.tofile(out/'skill.bin');diagnostic['replay_sha256']=sha(out/'skill.bin');atomic_json(out/'skill.json',diagnostic)
                    c['skill_100']=brief(diagnostic['metrics'][2])
                    manifest['items']=[x for x in manifest['items'] if x['piece']!='skill']+[dict(piece='skill',metric=c['skill_100'],checkpoint_song=key)]
                    atomic_json(folder/'replay_manifest.json',manifest)
                except InterruptedError:
                    c['evaluation_steps']+=E.last_steps;c['skill_100_incomplete']=True
                c['elapsed_s']=time.monotonic()-song_start
