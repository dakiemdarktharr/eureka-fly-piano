...
NAMES=([f'log_synapse_CPG_{i}' for i in range(18)]+[f'log_synapse_MN_{i}' for i in range(6)]+
       [f'log_gate_{i}' for i in range(6)]+[f'press_extension_{i}' for i in range(6)]+['press_advance_s'])
LOW=np.array([-.69]*24+[-1.4]*6+[-.25]*6+[0.])
HIGH=np.array([.69]*24+[1.4]*6+[.5]*6+[.08])
INITIAL=np.zeros(37);INITIAL[-1]=.02
    ...
    def rollout(self,theta,notes,duration,record=False,batched=True,cancel=None):
        ...
        for j in range(n):
            ...
            for l,seq in enumerate(seqs):
                ...
                idx=e['pitch']-21;h=self.hover[l,idx];p=self.press[l,idx]
