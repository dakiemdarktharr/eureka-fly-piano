...
def skill_pass(m):return m['precision']>=.95 and m['recall']>=.95
def sequence_pass(m):return m['precision']>.85 and m['recall']>=.85 and m['f1']>=.85
...
def aggregate(results):
    ...
    return dict(precision=p,recall=r,f1=2*p*r/(p+r) if p+r else 0,matched=tp,target_notes=nt,actual_notes=na)
...
def run(minutes=60,workers=3,run_id=None):
    ...
    def evaluate(pool,c,theta,task_seeds,cutoff,level=0,count=6):
        ...
        steps=sum(r['physics_steps'] for r in rr);total_steps+=steps;c['evaluation_steps']+=steps
    ...
    try:
        with concurrent.futures.ProcessPoolExecutor(workers,mp_context=multiprocessing.get_context('spawn'),initializer=init) as pool:
            for c in cells:
                ...
                if rr:
                    c['initial_validation']=aggregate(rr);c['best_validation']=c['initial_validation']['f1']
            ...
            while time.monotonic()<train_deadline and not (folder/'stop.request').exists():
                for c in cells:
                    ...
                    steps=sum(r['physics_steps'] for r in rr);total_steps+=steps;c['training_steps']+=steps;c['optimization_wall_s']+=time.monotonic()-tick
                    ...
                    if vr:
                        vm=aggregate(vr);value=vm['f1'];c['validation']=vm
...
def export(run_id):
    ...
    e=Engine(0,'adaptive');notes,d=e.synthetic(90001,count=6);todo=[('skill',notes,d)]
