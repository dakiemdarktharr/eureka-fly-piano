...
NAMES=([f'log_synapse_CPG_{i}' for i in range(18)]+[f'log_synapse_MN_{i}' for i in range(6)]+
       [f'log_gate_{i}' for i in range(6)]+[f'press_extension_{i}' for i in range(6)]+[f'MN_excitability_{i}' for i in range(6)]+[f'lateral_bias_mm_{i}' for i in range(6)]+['press_advance_s'])
LOW=np.array([-.69]*24+[-1.4]*6+[-.25]*6+[-50.]*6+[-.275]*6+[0.])
HIGH=np.array([.69]*24+[1.4]*6+[.5]*6+[50.]*6+[.275]*6+[.08])
INITIAL=np.zeros(49);INITIAL[-1]=.02
...
class Engine:
    def __init__(self,seed=0,variant='adaptive',dt=.0002):
        ...
        self.template=neural.MotorNetwork(seed=seed,variant='randomized' if variant=='rewired' else 'full',dt=.002)
    ...
    def synthetic(self,seed,level=0,count=6):
    ...
    def rollout(self,theta,notes,duration,record=False,batched=True,cancel=None):
        ...
        for j in range(n):
            ...
            amp=net.step(500,-1,feedback);gate=np.clip(8*amp*np.exp(theta[24:30]),0,1)
            ...
            for l,seq in enumerate(seqs):
                ...
                blend=gate[l]*(1+theta[30+l]) if e['start']-theta[-1]<=t<e['end'] else 0.
            ...
            if record and t+.001>=nextframe:
                frames.append(np.concatenate([w.pose().ravel(),d.qpos[w.key_qids],net.r[net.dn],net.r[net.cpg],amp,forces,gate]).astype('<f4'));nextframe+=1/24
        ...
        if record:
            g=len(self.geoms);result.update(piece='skill',variant=self.variant,seed=self.seed,fps=24,frame_count=len(frames),geom_count=g,stride=g*12+126,layout={'pose':[0,g*12],'key_displacement':[g*12,88],'DN':[g*12+88,2],'CPG':[g*12+90,18],'MN':[g*12+108,6],'contact_force':[g*12+114,6],'gate':[g*12+120,6]},dt=self.dt,neural_dt=.002,dtype='little-endian float32',controller='Hybrid: learned effective synaptic gains + motor gate; engineered score-to-IK; 150 ms lookahead',parameters=theta.tolist())
