...
<div class="campaignHead"><div><span class="eyebrow">THÍ NGHIỆM CÓ NGÂN SÁCH</span><h1>Một giờ. Bao nhiêu trải nghiệm?</h1></div><span id="jobState" class="pill">Đang đọc…</span></div>
<p id="jobMessage">Tốc độ phát video không làm tăng thời gian huấn luyện.</p>
<div class="statRow"><div><b id="realTime">—</b><span>Thời gian thực / ngân sách</span></div><div><b id="simTime">—</b><span>Toàn chiến dịch · 9 mô hình, train + đánh giá</span></div><div><b id="rtf">—</b><span>Giây mô phỏng / giây thực</span></div><div><b>8.766×</b><span>Mục tiêu 1 năm / 1 giờ</span></div></div>
<div class="progress"><i id="progressFill"></i></div>
<div class="experimentGrid"><div><canvas id="learningChart" height="190" aria-label="F1 validation theo thời gian"></canvas><p class="smallText">F1 validation của seed 0; điểm đo mỗi thế hệ, không phải test. Trục X: thời gian chiến dịch chung.</p></div><div><table id="resultsTable"><thead><tr><th>Mạng / seed</th><th>P</th><th>R</th><th>F1</th><th>Thế hệ</th><th>Trải nghiệm học</th></tr></thead><tbody></tbody></table></div></div>
<div class="jobControls"><label>Ngân sách tổng <select id="minutes"><option value="60">60 phút</option><option value="15">15 phút</option><option value="5">5 phút</option></select></label><button id="startTraining" class="primary">Tạo chiến dịch mới</button><button id="stopTraining">Dừng và đánh giá</button><a href="/api/export" target="_blank">Nhật ký JSON ↗</a><a href="/paper/manuscript.pdf" target="_blank">Paper v4 ↗</a><a href="/paper/reviewer.pdf" target="_blank">Giới hạn & phương án ↗</a></div>
...
js=js[:a]+'''async function load(song,variant='adaptive'){
 ...
 score=s;replay=r;frames=array;state.song=song;state.variant=variant;state.ready=true;state.time=0;audioCursor=0;$('seek').max=r.duration;$('sheet').hidden=song==='skill';$('midi').hidden=song==='skill';if(song!=='skill'){$('sheet').href=s.source_pdf;$('midi').href=`/data/${song}_reference.mid`}
